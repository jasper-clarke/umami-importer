...
website_event_columns = [
    ...
    'page_title', 'event_type', 'event_name', 'visit_id', 'tag', 
    'fbclid', 'gclid', 'li_fat_id', 'msclkid', 'ttclid', 'twclid', 
    'utm_campaign', 'utm_content', 'utm_medium', 'utm_source', 
    'utm_term', 'hostname'
]

# Write the website_event data to a new CSV file
with open('website_event_table.csv', mode='w', newline='') as file:
...
session_columns = [
    'session_id', 'website_id', 'browser', 'os', 'device',
    'screen', 'language', 'country', 'region', 'distinct_id',
    'city', 'created_at'
]

# Write the session data to a new CSV file
with open('session_table.csv', mode='w', newline='') as file:
